...
# PLACE ALL ERROR HANDLING HERE IN SAFE_POST
def safe_post(url, headers, json_payload):
    ...
    for attempt in range(retries):
        ...
        if attempt < retries - 1:  # If this is not the last attempt, wait before retrying
            time.sleep(10)
        else:
...
def cek_user(index):
    ...
    headers = headers_set.copy()  # Create a copy of headers_set so as not to change the global variable
...
def check_and_complete_tasks(index, headers):
    ...
    for task in tasks['data']['campaignTasks']:
        ...
        if task['name'] == "Follow telegram channel" and task['status'] == "Pending":
            ...
            continue  # Skip task if the name is "Follow telegram channel" and the status is "Pending"
    ...
    # Check updated tasks after moving to verification
...
def main():
    ...
    print("\rFetching list of valid accounts...", end="", flush=True)
